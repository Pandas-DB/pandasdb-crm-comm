...
def validate_contact_method(contact_method):
    """Validate contact method structure"""
    if not isinstance(contact_method, dict):
        return "Contact method must be an object"
    
    if 'type' not in contact_method:
        return "type field is required"
    
    if 'value' not in contact_method:
        return "value field is required"
    
    valid_types = ['phone', 'email', 'whatsapp', 'telegram']
...
def lambda_handler(event, context):
    ...
    try:
        ...
        return create_response(201, convert_decimals(response_data))
...
def check_contact_method_exists(contact_type, value):
    ...
    try:
        ...
        response = contact_methods_table.query(
            ...
            KeyConditionExpression='type_value = :type_value',
            ExpressionAttributeValues={':type_value': type_value}
        )
